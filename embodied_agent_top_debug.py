...
#LIVING_ROOM_SPAWN_XZ = (-3.2, -3.6)
#LIVING_ROOM_SPAWN_XZ = (-4.6, -0.8)
LIVING_ROOM_SPAWN_XZ = (-2.4, -1.96)
# 验证「近 view 跳过 Stage1」：True 时出生点落在 view#111 旁约 1m 处
STAGE1_DEBUG_SPAWN_NEAR_VIEW = False
STAGE1_DEBUG_VIEW_XZ = (-3.172, -0.792)
STAGE1_DEBUG_SPAWN_OFFSET_M = 1.0
...
ARRIVE_DEBUG = True
SERVO_DEBUG = True
STAGE3_SAVE_GIF = True
STAGE3_GIF_INTERVAL = 2
STAGE3_FAIL_SAVE_LAST_N = 20
STAGE3_DEBUG_ROOT = "/autodl-tmp"
STAGE1_SAVE_GIF = True
STAGE1_GIF_INTERVAL = 2
...
STAGE3_LOCK_BBOX_BY_TARGET = {"door": 0.05, "chair seat": 0.04, "bed": 0.10}
...
# 床：bulky_locked 用 1.68m 会在 ~1.4m 误 hold；逼近/到达用更严深度 + 前方探针一致性
STAGE3_BED_APPROACH_TARGET_DEPTH = 1.05
STAGE3_BED_BULKY_ARRIVE_MAX_DEPTH = 1.12
STAGE3_LOCK_FAST_CONF = 0.72
# 椅等 compact：远处 conf 高但 bbox 很小，仍应 1 帧锁定并走 lock_approach
STAGE3_LOCK_HIGHCONF_MIN_BBOX = 0.008
STAGE3_LOCK_HIGHCONF_CX_LO = 0.22
STAGE3_LOCK_HIGHCONF_CX_HI = 0.78
STAGE3_LOCKED_CX_JUMP_MAX = 0.26
STAGE3_LOCKED_CX_JUMP_MIN_CONF = 0.72
STAGE3_DISABLE_RECOVERY_WHEN_TARGET_VISIBLE = True
...
NAV_DEBUG = True
STAGE1_DEBUG = True
STAGE1_TOPO_VIZ_PATH = "/autodl-tmp/stage1_topo_{target}_{ts}_{tag}.png"
NAV_TRACE_VIZ_PATH = "/autodl-tmp/navigation_trace_{target}_{ts}.png"
...
LABEL_CONFLICT_WIN_MARGIN = 1.08
STAGE3_DISAMBIG_MARGIN = 1.04
STAGE3_BED_DISAMBIG_MARGIN = 1.15
# 找床时：沙发 conf 接近 bed（差 < gap）则视为互误，不采纳 bed
STAGE3_BED_RIVAL_CONF_GAP = 0.12
STAGE3_BED_SEMANTIC_WIN_MARGIN = 1.06
...
SHAPE_PRIORS = {
    "sofa": {"min_area": 0.012, "max_area": 0.45, "aspect": (1.15, 5.5)},
    ...
    "bed": {"min_area": 0.010, "max_area": 0.50, "aspect": (0.75, 4.2)},
}
...
def place_agent_on_navmesh(sim, random_spawn=False, living_room_spawn=True, agent_idx=0):
    ...
    if pathfinder.is_loaded:
        if random_spawn:
            if STAGE1_DEBUG_SPAWN_NEAR_VIEW:
                vx, vz = STAGE1_DEBUG_VIEW_XZ
                ox, oz = STAGE1_DEBUG_SPAWN_OFFSET_M, 0.0
                raw = np.array([vx + ox, 0.0, vz + oz], dtype=np.float32)
                state.position = pathfinder.snap_point(raw)
                print(
                    f"[SPAWN_DEBUG] near_view raw=[{raw[0]:.2f},{raw[2]:.2f}] "
                    f"snapped=[{state.position[0]:.2f},{state.position[2]:.2f}] "
                    f"view={STAGE1_DEBUG_VIEW_XZ}"
                )
            elif living_room_spawn:
...
def make_sim(random_spawn=True, living_room_spawn=True):
    ...
    if sim.pathfinder.is_loaded:
        ...
        print(
            f"Stage1 修复: recovery={'off' if STAGE1_DISABLE_RECOVERY else 'on'} "
            f"oscillation={'off' if STAGE1_DISABLE_OSCILLATION else 'on'} "
            f"heading_align>{STAGE1_HEADING_ALIGN_DEG}° "
            f"wp_progress={STAGE1_WP_PROGRESS_M}m "
            f"spawn_yaw_align={STAGE1_SPAWN_ALIGN_YAW} "
            f"s1_goal=region_center view_in_path={STAGE1_VIEW_YAW_IN_PATH} "
            f"skip_bad_wp={STAGE1_SKIP_BAD_WP} near_open≤{STAGE1_NEAR_OPEN_MAX_D + STAGE1_NEAR_OPEN_DIST_SLACK_M}m "
            f"geo≤{STAGE1_NEAR_OPEN_MAX_GEODESIC_M}m "
            f"rejoin_open={STAGE1_BUG2_REJOIN_OPEN_FINISH} "
            f"anti_backtrack={STAGE1_ANTI_BACKTRACK} "
            f"α {STAGE1_SEMANTIC_ALPHA_FAR}→{STAGE1_SEMANTIC_ALPHA_NEAR} "
            f"debug_near_view={STAGE1_DEBUG_SPAWN_NEAR_VIEW} "
            f"locomotion={STAGE1_LOCOMOTION_ENABLE}"
        )
    return sim
...
def _stage1_on_path_exhausted(
    ...
    if replans < int(STAGE1_NEAR_BLOCKED_MAX_REPLANS):
        if follower.set_goal(pos, goal, trace=trace, nlog=nlog):
            ...
            if dbg is not None:
                dbg.bump_extra("replans")
                dbg.bump_extra("near_blocked_replans")
    ...
    if _stage1_rewind_path_for_blocked_follow(follower, nlog=nlog):
        ...
        steer_state.clear_commit()
        if dbg is not None:
            dbg.bump_extra("near_blocked_rewind")
        if (
...
def _det_nav_weight(det):
    """导航消歧：conf × bbox 占比，抑制远景小框误检。"""
    if not det.get("found"):
        return 0.0
    br = float(det.get("bbox_ratio", 0.05))
    return float(det["confidence"]) * min(br * 8.0, 1.0)


def _det_bbox_aspect(det):
...
def _bed_sofa_disambig_reject(target_en, det, alt):
    """找床时抑制「沙发当床」：倍率 + 绝对 conf 差 + 语义权重三重闸门。"""
    if str(target_en or "") != "bed" or not det.get("found") or not alt.get("found"):
        return False, None
    dc = float(det["confidence"])
    ac = float(alt["confidence"])
    if ac >= dc * STAGE3_BED_DISAMBIG_MARGIN:
        return True, "rival_conf_margin"
    if ac >= dc - STAGE3_BED_RIVAL_CONF_GAP:
        return True, "rival_conf_gap"
    dw = _det_nav_weight(det)
    aw = _det_nav_weight(alt)
    if aw >= dw * STAGE3_BED_SEMANTIC_WIN_MARGIN:
        return True, "rival_semantic"
    # 沙发框往往更「横宽」：床检在强沙发竞争下要求略高的横纵比
    asp = _det_bbox_aspect(det)
    if asp is not None and ac >= dc * 1.02 and asp < 0.95:
        return True, "rival_shape"
    return False, None
...
def detect_target_disambiguated(detector, rgb, target_en):
    ...
    if not alt.get("found"):
        return det
    reject, reason = _bed_sofa_disambig_reject(target_en, det, alt)
    if not reject and str(target_en or "") != "bed":
        margin = STAGE3_DISAMBIG_MARGIN
        if float(alt["confidence"]) >= float(det["confidence"]) * margin:
            reject, reason = True, "rival_conf_margin"
    if reject:
        out = _empty_det()
        out["disambig_rejected"] = rival
        out["disambig_reason"] = reason
...
@dataclass
class NavDebugTracker:
    """分阶段导航 debug 统计，写入 navigation_log。"""

    stage: str = ""
    step_begin: int = 0
    steps: int = 0
    actions: Counter = field(default_factory=Counter)
    action_source: Counter = field(default_factory=Counter)
    recovery_triggers: int = 0
    stuck_max: int = 0
    extras: dict = field(default_factory=dict)

    def begin(self, stage, act_total):
        self.stage = stage
        self.step_begin = int(act_total)
        self.steps = 0
        self.actions = Counter()
        self.action_source = Counter()
    ...
    def bump_extra(self, key, n=1):
        self.extras[key] = int(self.extras.get(key, 0)) + n

    def record_executed(self, action, source="propose"):
        self.steps += 1
        self.actions[str(action)] += 1
        self.action_source[str(source)] += 1

    def format_summary(self, act_total=None):
        lines = [f"  [DEBUG {self.stage}]"]
        if act_total is not None:
            lines.append(
                f"    steps={self.steps} (global {self.step_begin}→{act_total})"
            )
        else:
            lines.append(f"    steps={self.steps}")
        if self.actions:
            act_s = ", ".join(f"{k}:{v}" for k, v in sorted(self.actions.items()))
            lines.append(f"    executed: {act_s}")
        if self.action_source:
            src_s = ", ".join(f"{k}:{v}" for k, v in sorted(self.action_source.items()))
            lines.append(f"    source: {src_s}")
        if self.recovery_triggers:
            lines.append(f"    recovery_triggers={self.recovery_triggers}")
        if self.stuck_max:
            lines.append(f"    stuck_steps_max={self.stuck_max}")
        for key in sorted(self.extras.keys()):
            lines.append(f"    {key}={self.extras[key]}")
        return "\n".join(lines)
...
def motion_controller(
    ...
    if enable_recovery and recovery.try_trigger(memory.stuck_steps):
        if dbg is not None:
            dbg.recovery_triggers += 1
        _nav_dbg_print(
            f"[RECOVERY] trigger stuck_steps={memory.stuck_steps} "
            f"phase={recovery.phase} remaining={recovery.remaining}"
        )
    ...
    if rec is not None:
        ...
        if allow_oscillation:
            ...
            source = "propose"
    if dbg is not None:
        dbg.record_executed(action, source)
        dbg.stuck_max = max(dbg.stuck_max, int(memory.stuck_steps))
...
class PersistentPathFollower:
    ...
    def set_goal(self, start, goal, trace=None, nlog=None):
        ...
        _nav_dbg_print(
            f"[PATHFINDER] points={len(self.path_points)} "
            f"geodesic={self.last_geodesic:.2f}",
            nlog=nlog,
        )
...
def path_follow_steering_persistent(
    ...
    _stage1_free_ctrl_dbg(left_d, front_d, right_d, mix_yaw, chosen_yaw, nlog=nlog)
    meta = {
    ...
    if NAV_DEBUG:
        _nav_dbg_print(
            f"[STEER] yaw={yaw:.2f} wp={math.degrees(wp_yaw):.1f} "
            f"mode={h_mode} guide={meta.get('guide_yaw_deg')} "
            f"mix={meta.get('mix_yaw_deg')} α={meta.get('mix_alpha')} "
            f"diff={math.degrees(diff):.1f} L/R/F={left_d:.2f}/{right_d:.2f}/{front_d:.2f} "
            f"intent={intent} off={meta.get('feasible_offset_deg')} "
            f"free={meta.get('feasible_free_d')} commit_rem={state.turn_commit_steps}"
        )
    if return_meta:
        return action, meta
    return action
...
def near_arrival_check(det, depth):
    """分项检查 near_arrival：conf / depth / center。"""
    out = {
        "ok": False,
        "td": None,
        "conf": 0.0,
    ...
    out["td"] = float(td)
    if out["conf"] < NAV_ARRIVE_CONF:
        out["fails"].append("conf")
    if out["td"] >= NAV_ARRIVE_DIST:
        out["fails"].append("depth")
    if out["cx"] < NAV_ARRIVE_CENTER_LO or out["cx"] > NAV_ARRIVE_CENTER_HI:
        out["fails"].append("center")
    out["ok"] = len(out["fails"]) == 0
    return out


def _arrival_debug_print(check, step, nlog=None, tier=None):
    if not ARRIVE_DEBUG:
        return
    if not check["found"]:
        _nav_dbg_print(f"[ARRIVE] step={step} no_detect", nlog=nlog)
        return
    td_s = f"{check['td']:.2f}" if check["td"] is not None else "n/a"
    if check.get("td_bbox") is not None and check["td"] is not None:
        if abs(float(check["td_bbox"]) - float(check["td"])) > 0.05:
            td_s = f"{check['td']:.2f}(bbox={check['td_bbox']:.2f})"
    if check["ok"]:
        tier_s = f" tier={tier or check.get('tier')}" if (tier or check.get("tier")) else ""
        status = f" OK{tier_s}"
    else:
        status = " (FAIL: " + ", ".join(check["fails"]) + ")"
    br_s = f" bbox={check.get('bbox_ratio', 0):.3f}" if "bbox_ratio" in check else ""
    _nav_dbg_print(
        f"[ARRIVE] step={step} conf={check['conf']:.3f} depth={td_s} "
        f"cx={check['cx']:.3f}{br_s}{status}",
        nlog=nlog,
    )


def _arrival_fail_reason(det, depth):
    """Stage3：未满足 near_arrival 时的主因（用于 debug 汇总）。"""
    check = near_arrival_check(det, depth)
    if not check["found"]:
        return "no_detect"
    if not check["fails"]:
        return "ok"
    parts = []
    if "conf" in check["fails"]:
        parts.append(f"conf_fail({check['conf']:.2f}<{NAV_ARRIVE_CONF})")
    if "depth" in check["fails"]:
        parts.append(f"depth_fail({check['td']:.2f}>={NAV_ARRIVE_DIST})")
    if "center" in check["fails"]:
        parts.append(f"center_fail(cx={check['cx']:.2f})")
    return ",".join(parts)


def stage1_motion_controller(agent, depth, memory, recovery, propose_action, dbg=None):
...
def _stage1_log_path_start(agent, goal, target_xz, follower, view=None, nlog=None):
    ...
    for line in lines:
        _stage1_dbg_print(line, nlog=nlog)
...
def run_persistent_path_follow(
    ...
    if not follower.set_goal(pos, goal, trace=trace, nlog=nlog):
        if dbg is not None:
            dbg.set_extra("exit", "path_plan_failed")
        return False
    ...
    if target_xz is not None:
        ...
        if bearing is not None:
            ...
            if n_pre > 0:
                _stage1_dbg_print(
                    f"  [Stage1 prealign] 转向 bearing→target turns={n_pre}",
                    nlog=nlog,
                )
                if dbg is not None:
                    dbg.bump_extra("prealign_turns", n_pre)
        ...
        if n_pre > 0:
            _stage1_dbg_print(
                f"  [Stage1 prealign] 转向 target_xz turns={n_pre}",
                nlog=nlog,
            )
            if dbg is not None:
                dbg.bump_extra("prealign_turns", n_pre)
    ...
    if dbg is not None:
        dbg.set_extra("path_points", len(follower.path_points))
        dbg.set_extra("geodesic", round(float(getattr(follower, "last_geodesic", 0)), 3))
        dbg.set_extra("wp_start_idx", follower.idx)
        if view is not None:
            vy = semantic_aim_yaw_from_view(view)
            if vy is not None:
                dbg.set_extra("view_aim_yaw_deg", round(math.degrees(vy), 1))
        if target_xz is not None:
            by = stage1_bearing_yaw(agent.state.position, target_xz)
            if by is not None:
                dbg.set_extra("bearing_yaw_deg", round(math.degrees(by), 1))
        if target_xz is not None:
            dbg.set_extra("dist_start", round(_dist_xz_to_target(pos, target_xz), 3))
            dbg.extras["dist_min"] = float(dbg.extras["dist_start"])
    ...
    prev_pos = pos.copy()
    s1_no_progress = 0
    s1_best_dist = float(dbg.extras.get("dist_start", 99.0)) if (
        dbg is not None and target_xz is not None
    ) else None
    wp_stuck_steps = 0
    ...
    while act.total < step_end:
        ...
        if STAGE1_DEBUG and act.total % 50 == 0:
            pos_now = agent.state.position
            d_to_goal = (
                _dist_xz_to_target(pos_now, target_xz) if target_xz else -1.0
            )
            cur_wp = follower.current_waypoint()
            if cur_wp is not None:
                d_to_wp = float(
                    np.linalg.norm(
                        np.array(
                            [float(pos_now[0]), float(pos_now[2])], dtype=np.float64
                        )
                        - np.array(
                            [float(cur_wp[0]), float(cur_wp[2])], dtype=np.float64
                        )
                    )
                )
            else:
                d_to_wp = 0.0
            _stage1_dbg_print(
                f"  [path step={path_steps} total={act.total}] "
                f"pos=[{pos_now[0]:.2f},{pos_now[2]:.2f}] "
                f"dist_to_goal={d_to_goal:.2f}m "
                f"dist_to_wp={d_to_wp:.2f}m "
                f"wp_idx={follower.idx}/{len(follower.path_points)}",
                nlog=nlog,
            )
        ...
        if target_xz is not None and neighborhood_m is not None:
            ...
            if dbg is not None:
                dbg.extras["dist_min"] = min(float(dbg.extras.get("dist_min", d)), d)
                dbg.set_extra("dist_last", round(d, 3))
            ...
            if d < near_limit:
                ...
                if (
                    nlog is not None
                    and STAGE1_NEAR_OPEN_AREA_GATE
                    and s1_near_open_streak == 0
                ):
                    _stage1_dbg_print(
                        f"  dist={d:.2f}m < {near_limit:.2f}m 但未达空旷闸门 "
                        f"({open_tag})，继续 path/绕障",
                        nlog=nlog,
                    )
        ...
        if follower.idx > wp_idx_prev and dbg is not None:
            dbg.bump_extra("wp_advances", follower.idx - wp_idx_prev)
            wp_idx_prev = follower.idx
        ...
        _stage1_mode_dbg(
            ...
            prev_moved,
            nlog=nlog,
            commit_rem=loco.commitment_remaining if loco.in_escape else None,
            phase=loco.phase if loco.in_escape else None,
            bug2_mline_d=bug2_mline_d,
            dist_goal=dist_target if (loco.in_escape or coord.in_mline_transit()) else None,
            dist_at_hit=loco.escape_start_dist if loco.in_escape else None,
        )
        ...
        if loco.in_escape:
            ...
            if door_ok:
                ...
                if u_exit:
                    ...
                    if STAGE1_REJOIN_REPLAN_PATH and follower.set_goal(
                        ...
                        path_steps = 0
                        if dbg is not None:
                            dbg.bump_extra("doorway_replans")
                        if nlog is not None:
                ...
                if dbg is not None:
                    dbg.bump_extra("wall_doorway_seek")
                msg = (
                ...
                if nlog is not None:
                    nlog(msg)
                _stage1_dbg_print(msg, nlog=nlog)
            elif (
                ...
                if dbg is not None:
                    if reacq_info.get("visible"):
                        dbg.bump_extra("wall_sem_visible")
                    if not reacq_info.get("reachable", True) and reacq_info.get(
                        "visible"
                    ):
                        dbg.bump_extra("wall_sem_not_reachable")
                if reacq_ok:
                    wall_used = loco.wall_side
                    ...
                    steer_state.clear_commit()
                    if dbg is not None:
                        dbg.bump_extra("wall_semantic_reacquire")
                    msg = (
                    ...
                    nlog is not None
                    and reacq_info.get("visible")
                    and STAGE1_DEBUG
                ):
                    _stage1_dbg_print(
                        f"  [WALL] 目标可见但未可达 ({reacq_tag}) 继续绕墙",
                        nlog=nlog,
                    )
            if loco.in_escape:
            ...
            if loco.in_escape and should_exit:
                ...
                if rejoin_open_finish:
                    ...
                    if dbg is not None:
                        dbg.bump_extra("rejoin_open_finish")
                    break
            ...
            if coord.transit_should_finish():
                ...
                if STAGE1_REJOIN_REPLAN_PATH and follower.set_goal(
                    ...
                    path_steps = 0
                    if dbg is not None:
                        dbg.bump_extra("transit_replans")
                msg = (
            ...
            if _skip:
                if nlog is not None and STAGE1_DEBUG:
                    _stage1_dbg_print(
                        f"  [COORD] 跳过 enter WALL ({skip_tag}) "
                        f"stall={loco.goal_stall_streak} no_move={loco.no_move_streak}",
                        nlog=nlog,
                    )
                if dbg is not None:
                    dbg.bump_extra("wall_enter_skipped")
                if (
                    ...
                    if dbg is not None:
                        dbg.bump_extra("doorway_turn_left")
        ...
        if steer_override is not None:
            ...
            _stage1_escape_dbg(
                ...
                nlog=nlog,
                phase=loco.phase,
                phase_rem=loco.phase_remaining,
            )
            _stage1_free_ctrl_dbg(
                left_d, front_d, right_d, frozen_yaw, yaw, nlog=nlog
            )
            meta = {
                "intent": intent,
            ...
            if dbg is not None:
                dbg.bump_extra(f"intent_{intent}")
        elif coord.in_mline_transit():
            ...
            _stage1_free_ctrl_dbg(
                left_d, front_d, right_d, align_yaw, align_yaw, nlog=nlog
            )
            if dbg is not None:
                dbg.bump_extra(f"intent_{intent}")
            ...
            _stage1_free_ctrl_dbg(
                left_d, front_d, right_d, mix_yaw, mix_yaw, nlog=nlog
            )
        ...
        if forward_guard_steps >= STAGE1_FORWARD_GUARD_STEPS:
            ...
            forward_guard_steps = 0
            if dbg is not None:
                dbg.bump_extra("forward_guard")

        if dbg is not None:
            dbg.bump_extra(f"intent_{meta['intent']}")
            if meta.get("intent") == "blocked_front":
                dbg.bump_extra("blocked_front_steps")

        def _propose():
            return proposed

        action = stage1_motion_controller(
            agent, depth, memory, recovery, _propose, dbg=dbg
        )
        yaw_diff_deg = float(
        ...
        _stage1_dbg_print(
            f"[PATH] mode={coord.mode} "
            f"dist_goal={dist_goal:.2f}{target_s}{dist_real_s} "
            f"dist_wp={dist_wp:.2f} "
            f"path_len={len(follower.path_points)} "
            f"wp_idx={follower.idx} "
            f"yaw_diff={yaw_diff_deg:.1f}{alpha_s} "
            f"delta_goal={delta:+.2f} "
        ...
        if (
            stage1_frames is not None
            and STAGE1_SAVE_GIF
            and path_steps % max(1, int(STAGE1_GIF_INTERVAL)) == 0
        ):
            obs_after = sim.get_sensor_observations()
            vis = obs_after["color"][:, :, :3].copy()
            ann = _stage1_annotate_frame(
                vis,
                act.total,
                action,
                dist_target,
                dist_wp,
                follower.idx,
                len(follower.path_points),
                yaw_diff_deg,
                meta.get("intent", ""),
                new_pos,
                moved_last=moved_dist,
            )
            stage1_frames.append(Image.fromarray(ann))
        _stage1_dbg_print(
            f"[FOLLOW] "
            f"pos=[{new_pos[0]:.2f},{new_pos[2]:.2f}] "
            f"next_wp=[{float(wp[0]):.2f},{float(wp[2]):.2f}] "
            f"move={moved_dist:.2f}",
            nlog=nlog,
        )
        prev_pos = new_pos
        ...
        last_action = action
        _nav_dbg_print(
            f"[NAV] step={act.total} dist={dist_goal:.2f} delta={delta:+.2f} action={action}",
            nlog=nlog,
        )
        prev_dist = dist_goal
        ...
        path_steps += 1
        if dbg is not None:
            dbg.bump_extra("path_loop_steps")

    if dbg is not None:
        dbg.set_extra("exit", exit_reason)
        dbg.set_extra("replans", replans)
        dbg.set_extra("path_finished_idx", follower.idx)
        dbg.set_extra("path_total_points", len(follower.path_points))
        dbg.set_extra("coordinator_mode_end", coord.mode)
        dbg.set_extra("controller_mode_end", coord.mode)
        dbg.set_extra("mline_transit_steps_last", int(coord.transit_steps))
        dbg.set_extra("bug2_last_exit", loco.last_exit_reason or "")
        dbg.set_extra("locomotion_escape_steps", int(loco.escape_total_steps))
        dbg.set_extra("locomotion_cumulative_move", round(loco.escape_cumulative_move, 3))
        dbg.set_extra("locomotion_no_move_streak", int(loco.no_move_streak))
        if target_xz is not None:
            dbg.set_extra("dist_end", round(_dist_xz_to_target(agent.state.position, target_xz), 3))
            dbg.set_extra(
                "dist_geo_end",
                round(
                    geodesic_distance_xz(
                        sim,
                        float(agent.state.position[0]),
                        float(agent.state.position[2]),
                        float(target_xz[0]),
                        float(target_xz[1]),
                    ),
                    3,
        ...
        dbg.set_extra("near_blocked_rewinds", near_blocked_rewinds)
    if exit_reason == "path_finished" and target_xz is not None:
        best_d = s1_best_dist
        if dbg is not None and "dist_min" in dbg.extras:
            dm = float(dbg.extras["dist_min"])
            best_d = dm if best_d is None else min(float(best_d), dm)
    ...
    return exit_reason in ("neighborhood", "neighborhood_open")


def _nav_debug_log(nlog, dbg, act_total):
    if NAV_DEBUG and dbg is not None and nlog is not None:
        nlog(dbg.format_summary(act_total))


def _nav_dbg_print(msg, nlog=None):
    if not NAV_DEBUG:
        return
...
def _stage1_dbg_print(msg, nlog=None):
    if not STAGE1_DEBUG:
        return
    print(msg, flush=True)
    if nlog is not None:
...
def _stage1_mode_dbg(
    ...
    if not STAGE1_DEBUG:
        return
    extra = ""
    if commit_rem is not None:
        extra += f" commit={commit_rem}"
    if phase:
        extra += f" phase={phase}"
    if bug2_mline_d is not None:
        extra += f" mline_d={bug2_mline_d:.2f}"
    if dist_goal is not None and dist_at_hit is not None:
        extra += f" dist={dist_goal:.2f}/{dist_at_hit:.2f}"
    msg = f"[MODE] mode={mode} front={front_d:.2f} moved={moved:.3f}{extra}"
    print(msg, flush=True)
    if nlog is not None:
        nlog(msg)


def _stage1_escape_dbg(left_d, right_d, action, nlog=None, phase=None, phase_rem=None):
    if not STAGE1_DEBUG:
        return
    extra = ""
    if phase:
    ...
    if phase_rem is not None:
        extra += f" phase_rem={phase_rem}"
    msg = f"[ESCAPE] left={left_d:.2f} right={right_d:.2f} action={action}{extra}"
    print(msg, flush=True)
    if nlog is not None:
        nlog(msg)


def _stage1_free_ctrl_dbg(left_d, front_d, right_d, desired_yaw, chosen_yaw, nlog=None):
    """Stage1 微避障：深度探针 + desired/feasible 航向（写入导航 log）。"""
    if not STAGE1_DEBUG:
        return
    msg_free = (
        f"[FREE] left={left_d:.2f} front={front_d:.2f} right={right_d:.2f}"
    )
    msg_ctrl = (
        f"[CTRL] desired={math.degrees(desired_yaw):.1f} "
        f"feasible={math.degrees(chosen_yaw):.1f}"
    )
    print(msg_free, flush=True)
    print(msg_ctrl, flush=True)
    if nlog is not None:
        nlog(msg_free)
        nlog(msg_ctrl)
...
class NavTraceRecorder:
    """导航轨迹记录：用于 debug 打印与最终可视化。"""

    trajectory: list = field(default_factory=list)
    waypoint_paths: list = field(default_factory=list)
    target_poses: list = field(default_factory=list)
    failed_positions: list = field(default_factory=list)
    current_stage: str = ""
    _seg_traj_begin: int = 0
    _seg_paths_begin: int = 0

    def set_stage(self, stage):
        self.current_stage = str(stage)

    def mark_segment_begin(self):
        """标记当前候选/阶段起点，便于 Stage1 切片可视化。"""
        self._seg_traj_begin = len(self.trajectory)
        self._seg_paths_begin = len(self.waypoint_paths)

    def segment_trajectory(self, stage=None):
        seg = self.trajectory[self._seg_traj_begin :]
        if stage is None:
            return seg
        return [p for p in seg if len(p) > 3 and p[3] == stage]

    def segment_waypoint_paths(self):
        return self.waypoint_paths[self._seg_paths_begin :]

    def record_step(self, x, z, step):
        self.trajectory.append(
            (float(x), float(z), int(step), str(self.current_stage))
        )
    ...
    def add_target(self, x, z, view_id=None, label=""):
    ...
    def add_failure(self, x, z, view_id=None, reason=""):
...
def make_tracked_pf_act(agent, act, recovery, trace=None, nlog=None):
    """包装 pf_act：记录 [MOVE]、[RECOVERY] 与轨迹点。"""

    def pf_act(name):
        prev_pos = np.array(agent.state.position, dtype=np.float64)
        agent.act(name)
        act.total += 1
        ...
        _nav_dbg_print(f"[MOVE] action={name} moved={move:.3f}", nlog=nlog)
        if recovery is not None and recovery.active():
            _nav_dbg_print(
                f"[RECOVERY] phase={recovery.phase} remaining={recovery.remaining}",
                nlog=nlog,
            )
        if trace is not None:
            trace.record_step(float(new_pos[0]), float(new_pos[2]), act.total)

    return pf_act


def visualize_navigation_trace(
    trace,
    output_path=None,
    title="Navigation Trace",
):
    """绘制 target pose、实际轨迹、waypoint path、失败位置。"""
    if not HAS_MATPLOTLIB:
        print("未安装 matplotlib，跳过导航轨迹可视化。可执行: pip install matplotlib")
        return None
    if not trace or (not trace.trajectory and not trace.target_poses):
        print("无轨迹数据，跳过导航可视化")
        return None
    ...
    for i, path_xz in enumerate(trace.waypoint_paths):
        if len(path_xz) < 2:
            continue
        px = [p[0] for p in path_xz]
        pz = [p[1] for p in path_xz]
        ax.plot(
            px,
            pz,
            color="#9b59b6",
            ...
            linestyle="--",
            zorder=2,
            label="waypoint path" if i == 0 else None,
        )

    if trace.trajectory:
        tx = [p[0] for p in trace.trajectory]
        tz = [p[1] for p in trace.trajectory]
        ax.plot(
            tx,
            tz,
            color="#1abc9c",
        ...
        ax.scatter(tx[0], tz[0], c="#27ae60", s=90, marker="o", zorder=6, label="start")
        ax.scatter(tx[-1], tz[-1], c="#2980b9", s=100, marker="*", zorder=6, label="end")

    for t in trace.target_poses:
        ax.scatter(
            t["x"],
            t["z"],
            s=200,
            ...
            linewidths=1.0,
            zorder=7,
            label="target pose" if t is trace.target_poses[0] else None,
        )
        vid = t.get("view_id")
        lbl = f"#{vid}" if vid is not None else ""
        ax.annotate(
            lbl,
            (t["x"], t["z"]),
            fontsize=8,
    ...
    for f in trace.failed_positions:
        ax.scatter(
            f["x"],
            f["z"],
            s=120,
            ...
            linewidths=2.5,
            zorder=8,
            label="failed" if f is trace.failed_positions[0] else None,
        )
        ax.annotate(
            f"{f.get('reason', '')[:12]}",
            (f["x"], f["z"]),
    ...
    legend_elems = [
        Line2D([0], [0], color="#1abc9c", linewidth=2, label="actual trajectory"),
        Line2D([0], [0], color="#9b59b6", linewidth=1.2, linestyle="--", label="waypoint path"),
        Line2D([0], [0], marker="X", color="w", markerfacecolor="#e74c3c", markersize=10, label="target pose"),
        Line2D([0], [0], marker="x", color="#e67e22", markeredgewidth=2, linestyle="None", label="failed"),
    ]
    ax.legend(handles=legend_elems, loc="upper left", fontsize=8)
    ...
    if output_path is None:
        output_path = "/autodl-tmp/navigation_trace.png"
    os.makedirs(os.path.dirname(output_path) or ".", exist_ok=True)
    fig.tight_layout()
    fig.savefig(output_path, dpi=150)
    plt.close(fig)
    print(f"导航轨迹图已保存: {output_path}")
    return output_path


def visualize_stage1_topo_overlay(
    topo_map,
    trace,
    output_path=None,
    title="Stage1: planned path vs trajectory",
    highlight_target_en=None,
    stage=None,
    tag="",
):
    """
    在语义拓扑图上叠加 Stage1：紫色虚线=habitat 规划 path，绿色实线=实际轨迹。
    用于区分 planner 错还是 controller 错。
    """
    if not HAS_MATPLOTLIB:
        print("未安装 matplotlib，跳过 Stage1 拓扑叠加图")
        return None
    if trace is None:
        return None

    stage_traj = trace.segment_trajectory(stage=stage) if stage else trace.segment_trajectory()
    seg_paths = trace.segment_waypoint_paths()
    if not stage_traj and not seg_paths:
        print("Stage1 无轨迹/path 数据，跳过拓扑叠加图")
        return None

    if not stage_traj:
        stage_traj = trace.segment_trajectory()
    ...
    centers = {
        rid: (node["center"][0], node["center"][1])
        for rid, node in topo_map.regions.items()
    }
    for rid, node in topo_map.regions.items():
        x0, z0 = centers[rid]
    ...
    for rid, node in topo_map.regions.items():
        ...
        face = "#fdebd0" if rid == best_rid else "#ebf5fb"
        edge = "#e67e22" if rid == best_rid else "#2980b9"
        ax.scatter(cx, cz, s=180, c=face, edgecolors=edge, linewidths=1.5, zorder=2)

    for i, path_xz in enumerate(seg_paths):
        if len(path_xz) < 2:
            continue
        px = [p[0] for p in path_xz]
        pz = [p[1] for p in path_xz]
        ax.plot(
            ...
            linestyle="--",
            zorder=4,
            label="planned path" if i == 0 else None,
        )

    if stage_traj:
        tx = [p[0] for p in stage_traj]
        tz = [p[1] for p in stage_traj]
        ax.plot(
            tx, tz,
            color="#1abc9c",
            linewidth=2.5,
            alpha=0.95,
            label="Stage1 trajectory",
            zorder=5,
        )
        ax.scatter(tx[0], tz[0], c="#27ae60", s=100, marker="o", zorder=7, label="S1 start")
        ax.scatter(tx[-1], tz[-1], c="#2980b9", s=120, marker="*", zorder=7, label="S1 end")

    seg_targets = trace.target_poses[-3:] if trace.target_poses else []
    for t in seg_targets:
        ax.scatter(
            t["x"], t["z"],
            s=220, c="#e74c3c", marker="X",
            edgecolors="white", linewidths=1.0, zorder=8,
            label="target" if t is seg_targets[0] else None,
        )

    legend_elems = [
        Line2D([0], [0], color="#9b59b6", linewidth=2, linestyle="--", label="planned path"),
        Line2D([0], [0], color="#1abc9c", linewidth=2.5, label="Stage1 trajectory"),
        Line2D([0], [0], marker="o", color="w", markerfacecolor="#ebf5fb", markeredgecolor="#2980b9", markersize=9, label="region"),
        Line2D([0], [0], marker="X", color="w", markerfacecolor="#e74c3c", markersize=10, label="target"),
    ]
    ax.legend(handles=legend_elems, loc="upper left", fontsize=8)
    suffix = f" | {tag}" if tag else ""
    ax.set_title(f"{title}{suffix}", fontsize=12)
    ax.set_xlabel("x (m)")
    ax.set_ylabel("z (m)")
    if output_path is None:
        output_path = "/autodl-tmp/stage1_topo_overlay.png"
    os.makedirs(os.path.dirname(output_path) or ".", exist_ok=True)
    fig.tight_layout()
    fig.savefig(output_path, dpi=150)
    plt.close(fig)
    print(f"Stage1 拓扑叠加图已保存: {output_path}")
    return output_path


def _stage1_annotate_frame(
    vis_rgb,
    step,
    action,
    dist_target,
    ...
    """在 RGB 帧上叠加 Stage1 path-follow 调试信息（供 GIF）。"""
    out = vis_rgb.copy()
    if out.dtype != np.uint8:
        out = (np.clip(out, 0, 1) * 255).astype(np.uint8)
    dt_s = f"{dist_target:.2f}" if dist_target is not None else "n/a"
    lines = [
        f"S1 step={step} act={action or 'hold'} intent={intent or ''}",
        f"pos=[{pos[0]:.2f},{pos[2]:.2f}] moved={moved_last:.3f}",
        f"dist_tgt={dt_s} dist_wp={dist_wp:.2f} wp={wp_idx}/{path_len}",
        f"yaw_diff={yaw_diff_deg:.1f}°",
    ]
    y0 = 22
    for line in lines:
        cv2.putText(
            out, line, (8, y0), cv2.FONT_HERSHEY_SIMPLEX, 0.45, (0, 255, 0), 1, cv2.LINE_AA
        )
        y0 += 20
    return out


def _save_stage1_debug_artifacts(stage1_frames, target_cn, ts_tag, view_id, reached, nlog):
    if not STAGE1_SAVE_GIF or not stage1_frames or not target_cn or not ts_tag:
        return
    status = "ok" if reached else "fail"
    tag = f"stage1_{target_cn}_{ts_tag}_v{view_id}_{status}"
    gif_path = os.path.join(STAGE3_DEBUG_ROOT, f"{tag}.gif")
    dur = 160 if reached else 120
    stage1_frames[0].save(
        gif_path, save_all=True, append_images=stage1_frames[1:], duration=dur, loop=0
    )
    if nlog is not None:
        nlog(f"  Stage1 GIF: {gif_path} ({len(stage1_frames)} 帧)")


def save_stage1_debug_viz(
    topo_map,
    trace,
    target_cn,
    ts_tag,
    ...
    """Stage1 结束后保存拓扑叠加图与 GIF（STAGE1_DEBUG / STAGE1_SAVE_GIF 时）。"""
    if STAGE1_DEBUG:
        dist_s = f"{dist_m:.2f}m" if dist_m is not None else "n/a"
        status = "ok" if reached else "fail"
        out = STAGE1_TOPO_VIZ_PATH.format(target=target_cn, ts=ts_tag, tag=tag)
        title = f"Stage1 {status} dist={dist_s}"
        path = visualize_stage1_topo_overlay(
            topo_map,
            trace,
            output_path=out,
            title=title,
            highlight_target_en=target_en,
            stage=stage,
            tag=tag,
        )
        if path and nlog is not None:
            nlog(f"  Stage1 debug 图: {path}")
    vid = view_id if view_id is not None else tag
    _save_stage1_debug_artifacts(stage1_frames, target_cn, ts_tag, vid, reached, nlog)
    return None
...
def visual_servo_action(det, depth):
    """
    检测到目标后：左/右转对齐 + 居中且远则前进（visual servoing）。
    未检测到目标时返回 None，由调用方 fallback。
    """
    if not det.get("found"):
        return None
    cx = float(det.get("center_x", 0.5))
    ok, td = near_arrival_met(det, depth)
    if ok:
        return None
    if cx < VISUAL_SERVO_CENTER_LO:
        return "turn_left"
    if cx > VISUAL_SERVO_CENTER_HI:
        return "turn_right"
...
def near_arrival_met(det, depth):
    check = near_arrival_check(det, depth)
    return check["ok"], check["td"]
...
def stage3_bed_front_consistent(td, front_d):
    """前方探针明显比 bbox 融合深度远 → 多为误检或斜看走廊，不能算到达。"""
    if td is None or front_d is None:
        return True
    return float(front_d) <= float(td) + STAGE3_BED_BULKY_FRONT_GAP


def stage3_lock_highconf_bulky(target_en, conf, br, cx):
    """床：高 conf + 中等框 + 中心在带内 → 快速锁定并 lock_approach。"""
    if not stage3_is_bed_target(target_en):
        return False
    if conf < STAGE3_LOCK_FAST_CONF:
        return False
    if br < STAGE3_BED_LOCK_MIN_BBOX * 0.75 or br > STAGE3_BED_LOCK_MAX_BBOX:
        return False
    # 床常从侧方入镜，中心可偏；比 compact 略宽
    return 0.15 <= float(cx) <= 0.88
...
def stage3_lock_highconf_compact(target_en, conf, br, cx):
    """椅等 compact：高 conf + 小框 + 中心在带内 → 允许快速锁定。"""
    key = str(target_en or "")
    if key == "door" or not stage3_is_compact_target(target_en):
        return False
    if conf < STAGE3_LOCK_FAST_CONF:
        return False
    if br < STAGE3_LOCK_HIGHCONF_MIN_BBOX:
        return False
    return STAGE3_LOCK_HIGHCONF_CX_LO <= float(cx) <= STAGE3_LOCK_HIGHCONF_CX_HI


def stage3_lock_bbox_ok(target_en, conf, br, cx):
    if br >= stage3_lock_bbox(target_en):
        return True
    return stage3_lock_highconf_compact(target_en, conf, br, cx)


def stage3_lock_frames_needed(target_en, conf, br, cx=0.5):
    need = int(STAGE3_LOCK_FRAMES)
    if str(target_en or "") == "door":
        if conf >= STAGE3_LOCK_FAST_CONF and stage3_lock_bbox(target_en) <= br <= STAGE3_DOOR_LOCK_MAX_BBOX:
            return 1
    elif stage3_lock_highconf_compact(target_en, conf, br, cx):
        return 1
    if stage3_lock_highconf_bulky(target_en, conf, br, cx):
        return 1
    return need


def stage3_filter_locked_det(det, last_det, track_locked):
    """锁定后抑制 cx 大幅跳变的误检（双目标左右抖）。"""
    if not track_locked or not det.get("found") or not last_det.get("found"):
        return det
    lc = float(last_det.get("center_x", 0.5))
    nc = float(det.get("center_x", 0.5))
    conf = float(det.get("confidence", 0.0))
    if abs(nc - lc) > STAGE3_LOCKED_CX_JUMP_MAX and conf < STAGE3_LOCKED_CX_JUMP_MIN_CONF:
        out = dict(last_det)
        out["synthetic"] = True
        return out
    return det
...
def stage3_recovery_allowed(front_d, det, target_en, agent_yaw=None, aim_yaw=None):
    ...
    if (
        STAGE3_DISABLE_RECOVERY_WHEN_TARGET_VISIBLE
        and det.get("found")
        and stage3_is_compact_target(target_en)
        and str(target_en or "") != "door"
    ):
        conf = float(det.get("confidence", 0.0))
        br = float(det.get("bbox_ratio", 0.0))
        cx = float(det.get("center_x", 0.5))
        if conf >= stage3_lock_conf(target_en) and (
            br >= stage3_lock_bbox(target_en)
            or stage3_lock_highconf_compact(target_en, conf, br, cx)
        ):
            return False
    if (
        STAGE3_DISABLE_RECOVERY_WHEN_TARGET_VISIBLE
        and det.get("found")
        and stage3_is_bed_target(target_en)
    ):
        conf = float(det.get("confidence", 0.0))
        br = float(det.get("bbox_ratio", 0.0))
        cx = float(det.get("center_x", 0.5))
        if conf >= stage3_lock_conf(target_en) and (
            br >= STAGE3_BED_LOCK_MIN_BBOX * 0.5
            or stage3_lock_highconf_bulky(target_en, conf, br, cx)
        ):
            return False
...
def near_arrival_check_stage3(
    ...
    if (
        ...
        depth_near = False
    bulky_depth_cap = float(STAGE3_BULKY_DEPTH_MAX)
    if stage3_is_bed_target(target_en):
        bulky_depth_cap = float(STAGE3_BED_BULKY_ARRIVE_MAX_DEPTH)
    bulky_locked = (
        bulky
        and locked
        and float(max_bbox) >= STAGE3_BULKY_MIN_MAX_BBOX
        and td < bulky_depth_cap
        and conf >= STAGE3_BULKY_MIN_CONF
        and cx_ok_edge
    )
    if bulky_locked and stage3_is_bed_target(target_en):
        if not stage3_bed_front_consistent(td, front_d):
            bulky_locked = False
        if conf < STAGE3_BED_ARRIVE_MIN_CONF:
            bulky_locked = False
        if det.get("disambig_rejected"):
            bulky_locked = False
    edge_locked = (
        bulky
        and locked
...
class Stage3TrackState:
    ...
    def update_raw(self, det, target_en=None):
        if det.get("found"):
            ...
            ) and not stage3_lock_highconf_bulky(target_en, conf, br, cx)
            lock_bbox_ok = stage3_lock_bbox_ok(target_en, conf, br, cx)
            if stage3_is_bed_target(target_en):
                lock_bbox_ok = lock_bbox_ok or stage3_lock_highconf_bulky(
                    target_en, conf, br, cx
                )
            if (
                not door_huge
            ...
            need = stage3_lock_frames_needed(target_en, conf, br, cx)
...
def stage3_needs_approach_forward(check, track, target_en=None):
    ...
    if stage3_is_bed_target(target_en):
        if td is None:
            return True
        if float(td) > float(STAGE3_BED_APPROACH_TARGET_DEPTH):
            return True
        if not stage3_bed_front_consistent(td, check.get("front_d")):
            return True
        return False
    if td is None:
        return True
...
def visual_servo_locked_approach(det, depth, front_d, track, target_en=None):
    ...
    approach_depth = float(STAGE3_APPROACH_TARGET_DEPTH)
    if stage3_is_bed_target(target_en):
        approach_depth = float(STAGE3_BED_APPROACH_TARGET_DEPTH)
    if td is None or td > approach_depth:
    ...
    if stage3_is_bed_target(target_en) and not stage3_bed_front_consistent(
        td, front_d
    ):
        if fd >= STAGE3_APPROACH_MIN_FRONT_M:
            return "move_forward", "lock_approach_front_gap"
        return ("turn_left" if cx < 0.5 else "turn_right"), "lock_approach_blocked"
...
def stage3_propose_action(
    ...
    use_lock_approach = track.locked
    if not use_lock_approach and effective.get("found"):
        hc = float(effective.get("confidence", 0.0))
        hbr = float(effective.get("bbox_ratio", 0.0))
        hcx = float(effective.get("center_x", 0.5))
        use_lock_approach = stage3_lock_highconf_compact(
            target_en, hc, hbr, hcx
        ) or stage3_lock_highconf_bulky(target_en, hc, hbr, hcx)
    if use_lock_approach and STAGE3_LOCK_APPROACH_UNTIL_DEPTH:
...
def motion_controller_stage3(
    ...
    if track is not None and STAGE3_DISABLE_RECOVERY_WHEN_LOCKED:
        if track.locked and track.lost_count <= STAGE3_LOST_USE_LAST_MAX:
            use_recovery = False
    if not use_recovery and recovery.active():
        recovery.clear()
    if use_recovery and recovery.try_trigger(memory.stuck_steps):
        if dbg is not None:
            dbg.recovery_triggers += 1
        _nav_dbg_print(
            f"[RECOVERY] trigger stuck_steps={memory.stuck_steps} "
    ...
    if rec is not None:
        ...
        if proposed is None:
            ...
            if overridden != proposed:
                ...
                action = proposed
                source = psrc
    if dbg is not None and action is not None:
        dbg.record_executed(action, source)
        dbg.stuck_max = max(dbg.stuck_max, int(memory.stuck_steps))
    elif dbg is not None and action is None:
        dbg.record_executed("hold", source)
...
def systematic_explore(
    ...
    for idx, goal in enumerate(sample_points):
        ...
        reached = run_persistent_path_follow(
            sim,
            agent,
            goal,
            act,
...
def navigate_coarse_to_xz(
    ...
    if dbg is not None:
        dbg.begin("Stage1_coarse", act.total)
        dbg.set_extra("dist_start", round(d0, 3))
        dbg.set_extra("step_end", int(step_end))
        dbg.set_extra("per_view_budget", int(step_end) - int(act.total))
        pos = agent.state.position
        dbg.set_extra("agent_pos", f"[{pos[0]:.2f},{pos[2]:.2f}]")
        dbg.set_extra("goal", f"[{goal[0]:.2f},{goal[2]:.2f}]")
        dbg.set_extra("heading_mode_start", h_mode0)
        yaw = yaw_from_rotation(agent.state.rotation)
        if bearing0 is not None:
            err_b, _ = heading_err_deg(yaw, bearing0)
            dbg.set_extra("heading_err_bearing_deg", round(err_b, 1))
        if view_yaw0 is not None:
            err_v, _ = heading_err_deg(yaw, view_yaw0)
            dbg.set_extra("heading_err_view_deg", round(err_v, 1))
        dbg.set_extra("semantic_alpha_start", round(stage1_semantic_alpha(d0), 2))
    if nlog is not None:
        pos = agent.state.position
        yaw = yaw_from_rotation(agent.state.rotation)
        err_b = 0.0
    ...
    if stage1_coarse_success(d0, "already_in_neighborhood"):
        if dbg is not None:
            dbg.set_extra("exit", "already_in_neighborhood")
            dbg.set_extra("dist_end", round(d0, 3))
            dbg.set_extra("stage1_skipped", True)
            dbg.set_extra("stage1_path_steps", 0)
        _nav_debug_log(nlog, dbg, act.total)
        if nlog is not None:
            nlog(
    ...
    path_ok = run_persistent_path_follow(
        sim,
        agent,
        goal,
        act,
    ...
    exit_r = str(dbg.extras.get("exit", "")) if dbg is not None else ""
    ok = bool(path_ok) and stage1_coarse_success(d, exit_r)
    if dbg is not None and "dist_end" not in dbg.extras:
        dbg.set_extra("dist_end", round(d, 3))
    _nav_debug_log(nlog, dbg, act.total)
    if nlog is not None:
        nlog(
...
def navigate_near_saved_view(
    ...
    if STAGE1_GOAL_FROM_REGION_CENTER and topo_map is not None:
        ...
        if pack is None:
            if dbg is not None:
                dbg.begin("Stage1_coarse", act.total)
                dbg.set_extra("exit", "approach_goal_not_navigable")
            _nav_debug_log(nlog, dbg, act.total)
            return False, None
        ...
        if dbg is not None:
            dbg.set_extra("approach_region", str(rid))
            dbg.set_extra("approach_target", f"[{target_xz[0]:.2f},{target_xz[1]:.2f}]")
            dbg.set_extra("approach_source", src)
            dbg.set_extra("view_pose_s23", f"[{vx:.2f},{vz:.2f}]")
    else:
        goal = snap_navigable(sim, vx, y, vz)
        if goal is None:
            if dbg is not None:
                dbg.begin("Stage1_coarse", act.total)
                dbg.set_extra("exit", "goal_not_navigable")
            _nav_debug_log(nlog, dbg, act.total)
            return False, None
        target_xz = [float(vx), float(vz)]
        if dbg is not None:
            dbg.set_extra("target_view", f"[{vx:.2f},{vz:.2f}]")
...
def recover_view_direction(
    ...
    if dbg is not None:
        dbg.begin("Stage2_viewpoint", act.total)
        yaw0 = yaw_from_rotation(agent.state.rotation)
        diff0 = (aim_yaw - yaw0 + math.pi) % (2 * np.pi) - np.pi
        dbg.set_extra("yaw_err_start_deg", round(math.degrees(diff0), 1))
        dbg.set_extra("tol_deg", round(math.degrees(tol), 1))
        dbg.set_extra("max_turns", int(max_turns))
    turns = 0
    ...
    if dbg is not None:
        dbg.set_extra("yaw_err_end_deg", round(math.degrees(diff_end), 1))
        dbg.set_extra("aligned", aligned)
        dbg.set_extra("turns_used", turns)
        dbg.set_extra("exit", exit_reason)
    _nav_debug_log(nlog, dbg, act.total)
    if nlog is not None:
        nlog(
...
def _stage3_annotate_frame(vis_rgb, det, step, action, check, front_d, yaw, aim_yaw, track=None):
    """在 RGB 帧上叠加 Stage3 调试信息（供 GIF / 失败帧）。"""
    out = vis_rgb.copy()
    td_s = f"{check['td']:.2f}" if check.get("td") is not None else "n/a"
    if check.get("ok"):
        fail_s = f"OK:{check.get('tier')}"
    elif check.get("fails"):
        fail_s = ",".join(check["fails"])
    else:
        fail_s = "no_detect"
    act_s = action if action is not None else "hold"
    lock_s = track.locked if track else False
    lost_s = track.lost_count if track else 0
    lines = [
        f"step={step} act={act_s} lock={int(lock_s)} lost={lost_s}",
        f"found={int(bool(det.get('found')))} conf={det.get('confidence', 0):.2f} "
        f"cx={det.get('center_x', 0.5):.2f} bbox={det.get('bbox_ratio', 0):.3f}",
        f"front={front_d:.2f} depth={td_s} arrive={fail_s}",
        f"yaw={yaw:.2f} aim_yaw={aim_yaw:.2f}",
    ]
    y0 = 22
    for line in lines:
        cv2.putText(out, line, (8, y0), cv2.FONT_HERSHEY_SIMPLEX, 0.45, (0, 255, 0), 1, cv2.LINE_AA)
        y0 += 20
    if det.get("bbox"):
        x0, y0, x1, y1 = det["bbox"]
...
def _stage3_servo_debug_print(
    ...
    aim_yaw = _stage3_aim_yaw(view, det, yaw)
    check = near_arrival_check_stage3(
        det,
        depth,
        front_d=front_d,
        locked=bool(track and track.locked),
        max_bbox=float(track.max_bbox) if track else 0.0,
        target_en=target_en,
    )
    if check["ok"]:
        arrive_s = f"OK:{check.get('tier')}"
    elif not check["found"]:
        arrive_s = "no_detect"
    else:
        arrive_s = ",".join(check["fails"])
    found = int(bool(det.get("found")))
    conf = float(det.get("confidence", 0.0))
    cx = float(det.get("center_x", 0.5))
    bbox_r = float(det.get("bbox_ratio", 0.0))
    lock_s = int(track.locked) if track else 0
    lost_s = track.lost_count if track else 0
    syn_s = int(det.get("synthetic", False))
    act_s = action if action is not None else "hold"
    _nav_dbg_print(
        f"[servo] step={step} raw={int(raw_found)} syn={syn_s} lock={lock_s} lost={lost_s} "
        f"found={found} conf={conf:.2f} cx={cx:.2f} bbox={bbox_r:.3f} front={front_d:.2f} "
        f"action={act_s} src={source} yaw={yaw:.2f} aim_yaw={aim_yaw:.2f} arrive={arrive_s}",
        nlog=nlog,
    )


def _save_stage3_debug_artifacts(stage3_frames, fail_ring, target_cn, ts_tag, view_id, success, nlog):
    if not target_cn or not ts_tag or view_id is None:
        return
    tag = f"stage3_{target_cn}_{ts_tag}_view{view_id}"
    if STAGE3_SAVE_GIF and stage3_frames:
        gif_path = os.path.join(STAGE3_DEBUG_ROOT, f"{tag}.gif")
        dur = 160 if success else 120
        stage3_frames[0].save(
            gif_path, save_all=True, append_images=stage3_frames[1:], duration=dur, loop=0
        )
        nlog(f"  Stage3 GIF: {gif_path} ({len(stage3_frames)} 帧)")
    if not success and fail_ring:
        n_fail = len(fail_ring)
        fail_dir = os.path.join(STAGE3_DEBUG_ROOT, f"{tag}_fail_last{n_fail}")
        os.makedirs(fail_dir, exist_ok=True)
        for i, im in enumerate(fail_ring):
            im.save(os.path.join(fail_dir, f"frame_{i:03d}.png"))
        nlog(f"  Stage3 失败帧: {fail_dir} ({n_fail} 张)")
...
def visual_servo_recovery(
    ...
    if recovery is None:
        recovery = RecoveryState()
    if dbg is not None:
        dbg.begin("Stage3_visual", act.total)
        dbg.set_extra("step_end", int(step_end))
        dbg.set_extra("target_en", str(target_en))
        dbg.set_extra("arrival_kind", stage3_arrival_kind(target_en))
        dbg.set_extra("policy", "lock+approach_until_depth+tier_arrive")

    track = Stage3TrackState()
    arrive_streak = 0
    scan_idx = 0
    max_conf = 0.0
    last_fail_reason = "no_detect"
    last_arrive_tier = None
    exit_reason = "budget_exhausted"
    arrive_conf_fail = 0
    arrive_depth_fail = 0
    arrive_center_fail = 0
    stage3_frames = []
    fail_ring = deque(maxlen=max(1, int(STAGE3_FAIL_SAVE_LAST_N)))
    s3_step = 0

    def _finish(success):
        _save_stage3_debug_artifacts(
            stage3_frames, fail_ring, target_cn, ts_tag, view_id, success, nlog
        )
        if frames is not None and stage3_frames:
            frames.extend(stage3_frames)
        return success
    ...
    while act.total < step_end:
        ...
        raw_det = stage3_filter_locked_det(raw_det, track.last_det, track.locked)
        raw_found = bool(raw_det.get("found"))
        track.update_raw(raw_det, target_en=target_en)
        effective = track.effective_det(raw_det)
        vis = rgb[:, :, :3].copy()
        if effective.get("bbox"):
            x0, y0, x1, y1 = effective["bbox"]
            color = (0, 200, 255) if effective.get("synthetic") else (0, 255, 255)
            cv2.rectangle(vis, (x0, y0), (x1, y1), color, 2)
            cv2.putText(
                vis,
                f"{effective['confidence']:.2f}",
                (x0, max(18, y0 - 6)),
                cv2.FONT_HERSHEY_SIMPLEX,
                0.5,
                color,
        ...
        if raw_found:
            if dbg is not None:
                dbg.bump_extra("detect_hit")
            if raw_det.get("disambig_rejected") and nlog is not None:
                nlog(
                    f"  [disambig] reject={raw_det.get('disambig_rejected')} "
                    f"reason={raw_det.get('disambig_reason')} "
                    f"bed={raw_det.get('disambig_target_conf', 0):.2f} "
                    f"rival={raw_det.get('disambig_rival_conf', 0):.2f}"
                )
            conf = float(raw_det["confidence"])
            max_conf = max(max_conf, conf)
            if dbg is not None:
                dbg.set_extra("last_conf", round(conf, 3))
                dbg.set_extra("last_cx", round(float(raw_det.get("center_x", 0.5)), 3))
        else:
            if dbg is not None:
                dbg.bump_extra("no_detect")
        if effective.get("synthetic") and dbg is not None:
            dbg.bump_extra("synthetic_det")
        ...
        if effective.get("found") and ARRIVE_DEBUG:
            _arrival_debug_print(check, act.total, nlog=nlog)
        ok = bool(check["ok"])
        if STAGE3_ARRIVE_REJECT_SYNTHETIC and effective.get("synthetic"):
            ok = False
        td = check["td"]
        if effective.get("found") and not ok:
            last_fail_reason = ",".join(check["fails"]) if check["fails"] else "unknown"
            if "conf" in check["fails"]:
                arrive_conf_fail += 1
            if "depth" in check["fails"]:
                arrive_depth_fail += 1
            if "center" in check["fails"]:
                arrive_center_fail += 1
            if dbg is not None:
                for tag in check["fails"]:
                    dbg.bump_extra(f"arrive_fail_{tag}")
        arrive_streak = arrive_streak + 1 if ok else 0
        if ok:
            last_arrive_tier = check.get("tier")
        if dbg is not None and ok:
            dbg.bump_extra("near_arrival_ok_step")
            dbg.bump_extra(f"arrive_tier_{check.get('tier')}")
        forwards_in_cand = int(act.forward) - int(cand_forward_begin)
        success_ok, success_reason = stage3_confirm_success(
            check, effective, track, arrive_streak, forwards_in_cand, target_en=target_en
        )
        if success_ok:
            exit_reason = "success"
            if dbg is not None:
                dbg.set_extra("arrive_streak", arrive_streak)
                dbg.set_extra("arrive_tier", last_arrive_tier)
                dbg.set_extra("arrive_depth", round(float(td), 3) if td is not None else None)
                dbg.set_extra("target_locked", track.locked)
                dbg.set_extra("forwards_in_cand", forwards_in_cand)
            _nav_debug_log(nlog, dbg, act.total)
            nlog(
        ...
        if (
            check.get("ok")
            and arrive_streak >= STAGE3_ARRIVE_STREAK
            and dbg is not None
        ):
            dbg.bump_extra(f"success_blocked_{success_reason}")
        ...
        def _propose():
            ...
            if src in ("search_scan", "lost_scan", "small_scan"):
                scan_idx += 1
            if dbg is not None and act_s is not None:
                dbg.bump_extra(f"s3_{src}_{act_s}")
            elif dbg is not None and act_s is None:
                dbg.bump_extra(f"s3_{src}")
            return act_s, src
        ...
        action, src = motion_controller_stage3(
            agent,
            depth,
            memory,
            recovery,
        ...
        ann = _stage3_annotate_frame(
            vis, effective, act.total, action, check, front_d, yaw_now, aim_yaw_step, track=track
        )
        fail_ring.append(Image.fromarray(ann))
        if STAGE3_SAVE_GIF and (s3_step % max(1, int(STAGE3_GIF_INTERVAL)) == 0):
            stage3_frames.append(Image.fromarray(ann))
        s3_step += 1

        if action is None:
            act.total += 1
            pn = agent.state.position
            _nav_dbg_print(f"[MOVE] action=hold moved=0.000", nlog=nlog)
        else:
            pf_act(action)

    if dbg is not None:
        dbg.set_extra("exit", exit_reason)
        dbg.set_extra("max_conf", round(max_conf, 3))
        dbg.set_extra("last_fail_reason", last_fail_reason)
        dbg.set_extra("arrive_streak_final", arrive_streak)
        dbg.set_extra("target_locked_final", track.locked)
        dbg.set_extra("max_bbox", round(track.max_bbox, 3))
        dbg.set_extra("forward_in_candidate", int(act.forward) - int(cand_forward_begin))
        dbg.set_extra("arrive_conf_fail", arrive_conf_fail)
        dbg.set_extra("arrive_depth_fail", arrive_depth_fail)
        dbg.set_extra("arrive_center_fail", arrive_center_fail)
        dbg.set_extra("stage3_gif_frames", len(stage3_frames))
        dbg.set_extra("stage3_fail_frames", len(fail_ring))
    _nav_debug_log(nlog, dbg, act.total)
    return _finish(False)


# ─── 阶段 2: 按语义地图导航 ───────────────────────────────
def navigate_to_target(sim, detector, topo_map, target_cn, max_steps=NAV_MAX_STEPS, save_gif=True):
    """
    ...
    pf_act = make_tracked_pf_act(agent, act, recovery, trace=nav_trace, nlog=nlog)

    nav_start_pos, nav_start_rot = _save_agent_pose(agent)
    nlog(
        f"[DEBUG] NAV_DEBUG={NAV_DEBUG} STAGE1_DEBUG={STAGE1_DEBUG} "
        f"ARRIVE_DEBUG={ARRIVE_DEBUG} "
        f"SERVO_DEBUG={SERVO_DEBUG} STAGE3_GIF={STAGE3_SAVE_GIF} "
        f"STAGE1_GIF={STAGE1_SAVE_GIF} "
        f"fail_last_n={STAGE3_FAIL_SAVE_LAST_N} "
        f"global_max={max_steps} per_view_budget={NAV_PER_VIEW_BUDGET} "
        f"geodesic_rank={NAV_USE_GEODESIC_VIEW_SCORE} "
        f"s1_reset_on_fail={NAV_RESET_ON_S1_FAIL} "
        f"起点=[{ax:.2f},{az:.2f}]"
    )
    nav_trace.record_step(ax, az, 0)
    ...
    if use_views:
        ...
        for rank, (view_id, nav_score, view) in enumerate(candidates, 1):
            ...
            nav_trace.add_target(ax, az, view_id=view_id, label=target_en)
            nav_trace.set_stage("S1_coarse")
            nav_trace.mark_segment_begin()
            dbg1 = NavDebugTracker() if NAV_DEBUG else None
            stage1_frames = [] if STAGE1_SAVE_GIF else None
            ...
            if not reached:
                ...
                pn = agent.state.position
                nav_trace.add_failure(
                    float(pn[0]), float(pn[2]), view_id=view_id, reason="S1_neighborhood"
                )
            ...
            nav_trace.set_stage("S2_viewpoint")
            dbg2 = NavDebugTracker() if NAV_DEBUG else None
            ...
            nav_trace.set_stage("S3_visual")
            dbg3 = NavDebugTracker() if NAV_DEBUG else None
            if visual_servo_recovery(
                ...
                trace_viz_path = NAV_TRACE_VIZ_PATH.format(target=target_cn, ts=ts_tag)
                visualize_navigation_trace(
                    nav_trace,
                    output_path=trace_viz_path,
                    title=f"Navigation OK {target_cn} view#{view_id}",
                )
                log.close()
                if save_gif and frames:
                    gif_path = f"/autodl-tmp/navigation_{target_cn}.gif"
                    frames[0].save(gif_path, save_all=True, append_images=frames[1:], duration=200, loop=0)
                    print(f"导航 GIF: {gif_path}")
                return True, frames, act.total
            topo_map.mark_view_failed(view_id, target_en, "no visual confirm at view")
            pn = agent.state.position
            nav_trace.add_failure(
                float(pn[0]), float(pn[2]), view_id=view_id, reason="S3_no_visual"
            )
        ...
        for rank, (rid, sem_score) in enumerate(region_cands, 1):
            ...
            nav_trace.set_stage("S1_region")
            nav_trace.add_target(float(goal[0]), float(goal[2]), view_id=str(rid), label=target_en)
            nav_trace.mark_segment_begin()
            dbg1 = NavDebugTracker() if NAV_DEBUG else None
            stage1_frames = [] if STAGE1_SAVE_GIF else None
            ...
            pn = agent.state.position
            dist_r = _dist_xz_to_target(pn, [float(goal[0]), float(goal[2])])
            save_stage1_debug_viz(
                topo_map,
                nav_trace,
                target_cn,
            ...
            if not reached:
                ...
                pn = agent.state.position
                nav_trace.add_failure(
                    float(pn[0]), float(pn[2]), view_id=str(rid), reason="S1_region"
                )
            ...
            if visual_servo_recovery(
                ...
                visualize_navigation_trace(
                    nav_trace,
                    output_path=trace_viz_path,
                    title=f"Navigation OK {target_cn} region {rid}",
                )
                log.close()
                if save_gif and frames:
                    gif_path = f"/autodl-tmp/navigation_{target_cn}.gif"
            ...
            pn = agent.state.position
            nav_trace.add_failure(
                float(pn[0]), float(pn[2]), view_id=str(rid), reason="S3_region"
            )
    ...
    visualize_navigation_trace(
        nav_trace,
        output_path=trace_viz_path,
        title=f"Navigation {target_cn}({target_en}) steps={act.total}",
    )
    log.close()
    if save_gif and frames:
        gif_path = f"/autodl-tmp/navigation_{target_cn}.gif"
...
def run_agent(user_input: str, sim=None, detector=None, topo_map=None, force_remap=False):
    ...
    success, frames, steps = navigate_to_target(sim, detector, topo_map, target_cn)
    ...
    return success, frames, sim, detector, topo_map
    ...
    try:
        while True:
            ...
            if text.lower() in ("q", "quit", "exit", "退出"):
                break
            success, _, sim, detector, topo_map = run_agent(text, sim, detector, topo_map)
    finally:
        if sim is not None:
            sim.close()
            print("仿真已关闭。")
